...
public List<Recruitment> searchbyname(String name,int pageNo){	
	...
	try{
		conn = DataBaseConn.getCon();
		//name,requirement,startTime,endTime,isending,position,workingPlace,idEnterpriseInformation,salary,number
		String sql2="select count(*) from enterpriserecruitment where name like  ? ";
		String sql="select IdEnterpriseRecruitment,enterpriserecruitment.Name,Requirement,Position,WorkingPlace,enterpriseinformation.IdEnterpriseInformation"+
					    ",Salary,Number,enterpriseinformation.name from  enterpriserecruitment,enterpriseinformation where enterpriseinformation.IdEnterpriseInformation=enterpriserecruitment.IdEnterpriseInformation and enterpriserecruitment.name like  ? limit ?,?";		
		ps = conn.prepareStatement(sql);		
	    ps.setString(1,"%"+name+"%");
	    ps.setInt(2, page);
	    ...
	    while(rs.next())
		{
	    	...
	    	recruitment.setName(rs.getString(2));
	    	recruitment.setRequirement(rs.getString(3));
	    	recruitment.setPosition(rs.getInt(4));
	    	recruitment.setWorkingPlace(rs.getString(5));
	    	recruitment.setIdEnterpriseInformation(rs.getInt(6));
	    	recruitment.setSalary(rs.getInt(7));
	    	recruitment.setNumber(rs.getInt(8));	
	    	recruitment.setCompany(rs.getString(9));
	    	recruitment.setSumcount(num);	
	    	recruitments.add(recruitment);
	    	System.out.println("haha");
		}
...
//分页前获取总条数
		...
		public List<Recruitment> searchbykeys(String keyword,
				String positionkey, String workkey, String moneykey, int i) {
			List<Recruitment> recruitments = new ArrayList<Recruitment>();	
			Connection conn = null;
			PreparedStatement ps = null;
			...
			try{
				...
				String sql="select IdEnterpriseRecruitment,enterpriserecruitment.Name,Requirement,Position,WorkingPlace,enterpriseinformation.IdEnterpriseInformation"+
					    ",Salary,Number,enterpriseinformation.name from  enterpriserecruitment,enterpriseinformation where enterpriseinformation.IdEnterpriseInformation=enterpriserecruitment.IdEnterpriseInformation and 1=1";
				if(positionkey!=null&&positionkey!=""){
					sql+=" and workingplace='"+positionkey+"'";
				...
				if(moneykey!=null&&moneykey!=""){
					if(moneykey.contains("-")){
						...
						System.out.println(array[0]);
						sql+=" and salary between  "+array[0]+" and "+array[1];
						sql2+=" and salary between  "+array[0]+" and "+array[1];
					}
					if(moneykey.contains("上")){
						System.out.println("aa");
						//String []array=moneykey.split("-");
						sql+=" and salary > 20000 ";
						sql2+=" and salary > 20000 ";
						System.out.println(moneykey);
					}
					if(moneykey.contains("下")){
						System.out.println("aa");
						sql+=" and salary< 1000 ";
						sql2+=" and salary< 1000 ";
						System.out.println(moneykey);
					}			
				}				
				sql+=" and enterpriserecruitment.name like ?  limit ?,?";
				sql2+=" and name like ?";
			    ...
			    System.out.println("hahasuccess");
			}catch(Exception e)
			{
				e.printStackTrace();
				throw new RuntimeException("按职业名称查询", e);
		...
		public  List<Recruitment> lookSomeAbout(int id){
			List<Recruitment> recruitments = new ArrayList<Recruitment>();
			...
			try{
				conn = DataBaseConn.getCon();				
				String sql="select * from  enterpriserecruitment,enterpriseinformation where POSITION   in ( select id from job where  industryid =?) and  enterpriserecruitment.idEnterpriseInformation=enterpriseinformation.idEnterpriseInformation and isending =1 order by startTime desc";				
				ps = conn.prepareStatement(sql);			
				ps.setInt(1, id);
				rs = ps.executeQuery();				
				while(rs.next()){									
			    	...
			    	recruitment.setSalary(rs.getInt(10));
			    	recruitment.setNumber(rs.getInt(11));
			    	recruitment.setCompany(rs.getString(14));
			    	
			    	recruitments.add(recruitment);			    					
				}
				System.out.println("按大种类查询成功");			    
			}catch(Exception e)
			{
				throw new RuntimeException("按大种类查询失败", e);
			}
			finally
			{
				//关闭连接
